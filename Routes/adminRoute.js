...
router.get("/projects", async (req, res) => {
  try{
    const projectData = await Project.find({},{name:-1,likedUser:-1}).sort({order:1})
    const data = projectData.map((each) => ({id:each._id, name:each.name, likes: each.likedUser.length}))
...
router.get("/project/:id", async (req, res) => {
  try{
    const projectData = await Project.findOne({_id:req.params.id}).sort({order:1})
